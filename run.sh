...
if [ ! -n "$FONT_VERSION" ]; then
    FONT_VERSION=$(curl -ks https://github.com/be5invis/Iosevka/releases/latest \
        | grep -Po '(?<=tag/v)[0-9.]*')
fi
...
if [ "$FONT_VERSION" == "dev" ]; then
    curl -ksSLo vdev.tar.gz --proto '=https' --tlsv1.2 https://github.com/be5invis/Iosevka/tarball/dev
else
    curl -ksSLO --proto '=https' --tlsv1.2 https://github.com/be5invis/Iosevka/archive/v${FONT_VERSION}.tar.gz
fi
